...
We used a linear regression model, which applies a best fit line to our explanatory variable, average number of people affected by plane crashes(x), and our response variable, the average freedom index(y). Our estimated regression model is y = 12.07 – 0.447x. Our intercept of 12.07 tells us that when a country’s freedom index is 0, the predicted number of people affected by plane crashes per year is 12.07. Our slope of -0.447 tells us that for each one unit increase in average freedom index score, the predicted average number of people affected by plane crashes per year decreases by 0.447.
